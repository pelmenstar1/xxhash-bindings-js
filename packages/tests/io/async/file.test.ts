  ...
  for (const lib of libs) {
    ...
    for (const preferMap of preferMapValues) {
      ...
      for (const options of [
        ...
        const actual = await fileAsync(options);
...
test.each<[VariantName, number | bigint]>([
  ...
  for (const lib of libs) {
    ...
    for (const preferMap of preferMapValues) {
      const actual = await fileAsync({ path: testData('onebyte'), preferMap });
...
test.runIf(fs.existsSync('/dev/zero')).each<[VariantName, number | bigint]>([
  ['xxhash32', 593485017],
...
test.each(variantNames.map((name) => [name]))(
  ...
  async (name) => {
    for (const lib of libs) {
      ...
      await expect(() =>
        fileAsync({ path: 123 as unknown as string }),
      ).rejects.toBeTruthy();
    }
  },
);
...
test.each(variantNames.map((name) => [name]))(
  ...
  async (name) => {
    for (const lib of libs) {
      ...
      await expect(() =>
        fileAsync({ path: './.should_not_exist' }),
      ).rejects.toBeTruthy();
    }
  },
);

...
type DirectoryToMapFactory = (
  lib: Lib,
  name: VariantName,
) => Lib[VariantName]['directoryToMapAsync'];
...
export function setupTests(directoryToMapFactory: DirectoryToMapFactory) {
  async function forEachOptions(
    ...
    block: (options: {
      lib: Lib;
      directoryToMapAsync: Lib[VariantName]['directoryToMapAsync'];
      path: string;
    }) => void | Promise<void>,
  ): Promise<void> {
    for (const lib of libs) {
      const directoryToMapAsync = directoryToMapFactory(lib, name);

      for (const suffix of PATH_SUFFIXES) {
        const path = testData(`${dirName}${suffix}`);

        await block({ lib, directoryToMapAsync, path });
      }
    }
  }
  ...
  test.each<[VariantName, Record<string, number | bigint>]>([
    ...
    return forEachOptions(
      name,
      'dir',
      async ({ lib, directoryToMapAsync, path }) => {
